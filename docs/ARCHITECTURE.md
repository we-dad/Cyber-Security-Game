...
Technical structure of **Cyber Hero**.
...
**Stage 4 — quiz.** `QuestionsSystem` loads `QuestionData` ScriptableObjects from resources, filters by category so each completed stage contributes three questions, and unlocks progression on a majority-correct result. Questions are authored as assets rather than in code, so wording can be revised without a rebuild.
